...
export default function Carrito() {
    ...
    return (
        <>
            ...
            {/* ─── MODAL LATERAL ─── */}
            {isCartOpen && (
                <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm transition-opacity" onClick={() => setIsCartOpen(false)}>
                    <div className="w-full sm:w-[450px] h-full bg-white shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-8" style={{ fontFamily: 'var(--font-montserrat)' }}>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button onClick={() => setMetodo('envio')} className={`py-2.5 rounded-xl border-2 font-bold text-sm transition-all ${metodo === 'envio' ? 'border-[var(--red)] bg-red-50 text-[var(--red)]' : 'border-neutral-200 text-neutral-500'}`}>Envío</button>
                                    <button onClick={() => setMetodo('retiro')} className={`py-2.5 rounded-xl border-2 font-bold text-sm transition-all ${metodo === 'retiro' ? 'border-[var(--red)] bg-red-50 text-[var(--red)]' : 'border-neutral-200 text-neutral-500'}`}>Retiro</button>
                                </div>
                            </div>
                            ...
                            {metodo === 'envio' && (
                                <div className="space-y-5 animate-fade-in">
                                    <div>
                                        ...
                                        <input type="text" placeholder="Para saber de quién es el pedido" className="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-[var(--red)] outline-none text-sm bg-white" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-neutral-800 mb-1.5 uppercase">Dirección</label>
                                        <input type="text" placeholder="Calle, número, piso y depto" className="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-[var(--red)] outline-none text-sm bg-white" />
                                    </div>
                                </div>
                            )}
                            ...
                            {metodo === 'retiro' && (
                                <div>
                                    <label className="block text-xs font-bold text-neutral-800 mb-1.5 uppercase">Tu nombre</label>
                                    <input type="text" placeholder="Para preparar tu pedido" className="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-[var(--red)] outline-none text-sm bg-white" />
                                </div>
                            )}

                            <div>
                                <label className="block text-xs font-bold text-neutral-800 mb-1.5 uppercase">Aclaraciones</label>
                                <textarea placeholder="Sin cebolla, timbre roto, etc." rows={2} className="w-full px-4 py-3 rounded-xl border border-neutral-300 focus:border-[var(--red)] outline-none text-sm resize-none bg-white" />
                            </div>
                        </div>
                        ...
                        <div className="p-6 border-t border-neutral-200 bg-neutral-50 pb-8">
                            ...
                            <button className="w-full py-4 rounded-full font-black text-sm text-white tracking-widest hover:scale-[1.02] shadow-lg" style={{ backgroundColor: 'var(--red)', fontFamily: 'var(--font-montserrat)' }}>
